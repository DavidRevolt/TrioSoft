...
class AppNetworkDataSourceImpl @Inject constructor(private val firestore: FirebaseFirestore) :
    ...
    override suspend fun create(point: NetworkPoint) {
        ...
        if (exists.isEmpty()) {
            ...
            exists[0].reference.update(TEMPERATURE_FIELD,point.temperature,HUMIDITY_FIELD,point.humidity).await()
        }
